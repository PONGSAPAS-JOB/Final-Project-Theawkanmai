...
if ($_SESSION['id_admin'] == "") {
    ...
    // Example query
    $sql = "SELECT id_manager, username, email, phone FROM manager";
    ...
    <body onload="init();">
        ...
        <div class="addplace ">
            ...
            <div style="width: 300px; padding: 20px; margin-left: 130px; white-space: nowrap; margin-top: 7px;">
                <a href="MemberMG.php" class="btn btn-warning">รายชื่อผู้ใช้งานทั่วไป</a>
            </div>
                ...
                <table class="table table-bordered" style="font-size: 15px;" id="placesTable">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">จำนวนสถานที่ท่องเที่ยวที่เป็นเจ้าของ</th>
                            <th scope="col">จำนวนร้านค้าหรือที่พักที่เป็นเจ้าของ</th>
                    ...
                    <tbody>
                        <?php
                        $index = 1; // Initialize $index before using it
                        while ($row = mysqli_fetch_assoc($result)) {
                            $area_count = $fetchdataManager->countOwnedArea($row['id_manager']); // Ensure this function is defined
                            $places_count = $fetchdataManager->countOwnedPlaces($row['id_manager']);
                        ?>
                            <tr>
